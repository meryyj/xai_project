...
from src.data.datasets import EMOTION_LABELS, RAFDBDataset, build_dataloaders
from src.data.transforms import build_transforms
from src.evaluation.deep_shap import DeepSHAPExplainer
...
def _build_deepshap_background(cfg: dict, xai_cfg: dict) -> torch.Tensor:
    deepshap_cfg = xai_cfg.get("deepshap", {})
    background_size = int(deepshap_cfg.get("background_size", 64))
    if background_size <= 0:
        raise ValueError("DeepSHAP background_size must be positive.")

    background_split = str(deepshap_cfg.get("background_split", "train")).lower()
    split_key = f"{background_split}_split"
    if split_key not in cfg.get("dataset", {}):
        raise ValueError(f"Unsupported DeepSHAP background split: {background_split}")

    eval_transform = build_transforms(cfg, split="test")
    dataset_cfg = cfg["dataset"]
    split_csv = Path(dataset_cfg["splits_dir"]) / dataset_cfg[split_key]
    background_dataset = RAFDBDataset(
        root=dataset_cfg["root"],
        split_csv=split_csv,
        transform=eval_transform,
    )

    sample_count = min(background_size, len(background_dataset))
    if sample_count <= 0:
        raise ValueError("DeepSHAP background dataset is empty.")

    seed = int(deepshap_cfg.get("background_seed", xai_cfg.get("random_seed", 42)))
    rng = np.random.default_rng(seed)
    indices = rng.choice(len(background_dataset), size=sample_count, replace=False).tolist()
    background_tensors = [background_dataset[int(idx)][0] for idx in indices]
    return torch.stack(background_tensors, dim=0)
...
def _build_explainers(
    ...
    methods: list[str],
    *,
    deepshap_background: torch.Tensor | None = None,
):
    perturbation_cfg = xai_cfg.get("perturbation_methods", {})
    default_grid_segments = int(perturbation_cfg.get("rows", 5)) * int(perturbation_cfg.get("cols", 5))
    perturbation = PerturbationExplainerConfig(
        segmentation_method=str(perturbation_cfg.get("segmentation_method", "grid")),
        rows=int(perturbation_cfg.get("rows", 5)),
        cols=int(perturbation_cfg.get("cols", 5)),
        slic_n_segments=int(perturbation_cfg.get("slic_n_segments", default_grid_segments)),
        slic_compactness=float(perturbation_cfg.get("slic_compactness", 10.0)),
        slic_sigma=float(perturbation_cfg.get("slic_sigma", 1.0)),
        mediapipe_model_asset_path=str(perturbation_cfg.get("mediapipe_model_asset_path", "")),
        mediapipe_refine_landmarks=bool(perturbation_cfg.get("mediapipe_refine_landmarks", True)),
        mediapipe_min_detection_confidence=float(
            perturbation_cfg.get("mediapipe_min_detection_confidence", 0.5)
        ),
        mediapipe_min_tracking_confidence=float(
            perturbation_cfg.get("mediapipe_min_tracking_confidence", 0.5)
        ),
        mediapipe_fallback_method=str(perturbation_cfg.get("mediapipe_fallback_method", "grid")),
    ...
    for method in methods:
        if method == "gradcam":
            ...
            if deepshap_background is None:
                raise ValueError("DeepSHAP background data was not prepared.")
            explainers[method] = DeepSHAPExplainer(
                model,
                device,
                deepshap_background,
                check_additivity=bool(deepshap_cfg.get("check_additivity", False)),
            )
        else:
...
def main() -> None:
    ...
    deepshap_background = None
    if "deepshap" in methods:
        deepshap_background = _build_deepshap_background(cfg_dict, xai_cfg)
        logger.info("DeepSHAP background tensor shape: %s", tuple(deepshap_background.shape))

    explainers = _build_explainers(
        model,
        device,
        xai_cfg,
        methods,
        deepshap_background=deepshap_background,
    )
